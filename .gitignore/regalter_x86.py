...
# Constant(s)
APP_NAME = "Regalter: GUI modifier for Windows owner and organization (32-bit)" # The default title for Regalter that
                                                                                # will appear on the title bar.

# Regalter is OS-architecture-specific, and this code is for the 32-bit version of Windows, so we will perform
...
if (ctypes.windll.shell32.IsUserAnAdmin() == 0):
    mbox(0,
        ...
        "Error",
        mboxconst.MB_OK | mboxconst.MB_ICONSTOP | mboxconst.MB_TASKMODAL)
    print ("Closed "+APP_NAME[0:APP_NAME.index(":")]+" without any errors.")
...
class Regalter:
    ...
    def __init__(self):
        ...
        if (path.isfile('assets\\uac_16x16.png')):
            ...
            self.change_btn=ttk.Button(self.root_window, text = 'Change', command = self.change_button, image = icon, compound = 'left', cursor='hand2')
        ...
        # without the shield icon.
            ...
            self.change_btn=ttk.Button(self.root_window, text = 'Change', command = self.change_button, cursor='hand2')
        ...
        self.owner_var.set(Initial_RegisteredOwner) # The initial value appearing in the RegisteredOwner entry will be the current value of
                                                    # RegisteredOwner from the Registry
                                                    
        self.org_var.set(Initial_RegisteredOrganization) # The initial value appearing in the RegisteredOrganization entry will be the current value of
        ...
        self.regowner = Initial_RegisteredOwner
        self.regorg = Initial_RegisteredOrganization
        ...
        self.winver_button.place(x=500, y=20, width=120)    
        self.owner_label.place(x=10, y=60)
        self.owner_entry.place(x=200, y=60, width = 420)
        self.org_label.place(x=10, y=110)
        self.org_entry.place(x=200, y=110, width = 420)
        self.change_btn.place(x=501, y=160, width = 120)
        self.update_btn.place(x=300, y=160, width = 125)
    ...
    def launch_winver_button(self):
        ...
        if (mbox(0,
            ...
            self.winver_button.configure(state='active')
            return
    ...
    def update_names_button(self):
        ...
        mbox(0,
                "Registered owner and registered organization names have been successfully changed. Click the Launch winver.exe button to view the changes.",
                "Update successful",
    ...
    def close_root_window(self):
        ...
        # reference to self.wmi_ob. Note that self.wmi_ob reference was created in the function winver_func(self).
        # Now, if the function close_root_window(self) is executed before function winver_func(self), Python will
        # raise an AttributeError because it will not be able to find the reference to the wmi.WMI() object, i.e,
        # self.wmi_ob. This will cause the code in the except block to be executed. The except block simply destroys
        # the root window and returns from the function close_root_window(self).
        try:
            wmi_ob = self.wmi_ob
            all_processes = wmi_ob.Win32_Process() # If self.wmi_ob has been declared (or winver_func(self) has been
                                                   # executed) then get all the currently running processes.
    ...
    def update_update_names_button_state(self):
        if (self.regowner != self.owner_var.get() or self.regorg != self.org_var.get()):
